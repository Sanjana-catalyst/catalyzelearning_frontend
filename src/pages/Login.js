...
import { useLogin } from '../contexts/LoginStore'; // Correct named import
...
function Login() {
  ...
  const { login, error } = useLogin(); // Directly use the custom hook

  const words = ['Ideas', 'Technologies', 'Learning', 'Opportunities'];

  const handleLogin = (e) => {
    e.preventDefault(); // Prevent default form submission
    login(username, password); // Call login function with credentials
  };
    ...
    <div className="h-[100vh]">
      <div className="flex flex-col md:flex-row h-[100vh] w-full items-center justify-center p-4 md:p-8 overflow-hidden">
        ...
        <form
          ...
          onSubmit={handleLogin} // Use the wrapper function here
        >
